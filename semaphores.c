...
int main()
{
    ...
    sem_t *mutex, *em_mutex, *dc_mutex, *cp_mutex, *mp_mutex; 
  
    if ((mutex = sem_open("examplesemaphore", O_CREAT, 0644, 3)) == SEM_FAILED) {
      perror("semaphore initilization");
      exit(1);
    }
  
    // Mutex for Elephant and Mice
    if ((em_mutex = sem_open("examplesemaphore", O_CREAT, 0644, 1)) == SEM_FAILED) {
      perror("semaphore initilization");
      exit(1);
    }

    // Mutex for dog and cat
    if ((dc_mutex = sem_open("examplesemaphore", O_CREAT, 0644, 1)) == SEM_FAILED) {
      perror("semaphore initilization");
      exit(1);
    }

    // Mutex for cat and parrot
    if ((cp_mutex = sem_open("examplesemaphore", O_CREAT, 0644, 1)) == SEM_FAILED) {
      perror("semaphore initilization");
      exit(1);
    }

    // Mutex for Mouse and Parrot.
    if ((mp_mutex = sem_open("examplesemaphore", O_CREAT, 0644, 1)) == SEM_FAILED) {
      perror("semaphore initilization");
      exit(1);
    }
